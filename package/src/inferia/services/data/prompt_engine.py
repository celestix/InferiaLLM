...
class PromptEngine:
    ...
    def _get_encoder(self, model_name: str = "gpt-3.5-turbo"):
        if model_name not in self._encoders:
            try:
                ...
                logger.warning(
                    f"Could not find encoder for model {model_name}, usage cl100k_base"
                )
    ...
    def process_prompt_from_content(
        ...
        from inferia.services.data.prompt_templates import PromptTemplate
    ...
    def check_token_budget(
        self, text: str, budget: int, model_name: str = "gpt-3.5-turbo"
    ) -> bool:
        """
    ...
    async def rewrite_prompt(self, prompt: str, goal: str = "clarity") -> str:
        ...
        """
        from inferia.services.data.prompt_rewriter import prompt_rewriter

        return await prompt_rewriter.rewrite(prompt, goal)

    async def assemble_context(
        self, query: str, collection_name: str, org_id: str, n_results: int = 3
    ) -> str:
        """
