...
async def create_api_key(
    key_data: ApiKeyCreate, request: Request, db: AsyncSession = Depends(get_db)
):
...
async def list_api_keys(
    request: Request,
    skip: int = Query(0, ge=0, description="Number of API keys to skip"),
    limit: int = Query(
        50, ge=1, le=100, description="Maximum number of API keys to return"
    ),
    ...
    keys_result = await db.execute(
        select(DBApiKey)
        .where(DBApiKey.org_id == user_ctx.org_id)
        .offset(skip)
        .limit(limit)
    )
...
async def revoke_api_key(
    ...
    result = await db.execute(
        select(DBApiKey).where(
            (DBApiKey.id == key_id) & (DBApiKey.org_id == user_ctx.org_id)
        )
    )
