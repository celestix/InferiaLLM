    ...
    def refresh_from_main_settings(self) -> None:
        ...
        try:
            ...
            if groq_key is not None:
                if groq_key:
                    logger.info(
                        f"[GuardrailSettings] Hydrating Groq key from config: {groq_key[:6]}..."
                    )
            ...
            if lakera_key is not None:
                if lakera_key:
                    logger.info(
                        f"[GuardrailSettings] Hydrating Lakera key from config: {lakera_key[:6]}..."
                    )
                self.lakera_api_key = lakera_key
        except ImportError:
            logger.error(
                "[GuardrailSettings] Failed to import config.settings for hydration"
            )
            pass
