...
from llm_guard.input_scanners import Toxicity, PromptInjection, Secrets, Code
...
class LLMGuardProvider(GuardrailProvider):
    ...
    def _init_input_scanners(self):
        ...
        if self.settings.enable_prompt_injection:
            scanners.append(
                PromptInjection(threshold=self.settings.prompt_injection_threshold)
            )
        ...
        if self.settings.enable_code_scanning:
            scanners.append(
                Code(languages=["Python", "Java", "JavaScript", "Go", "C++"])
            )
    ...
    def _init_output_scanners(self):
        ...
        if banned_list:
            scanners.append(
                BanSubstrings(substrings=banned_list, match_type="str")
            )  # simple string match
    ...
    async def scan_input(
        ...
        if custom_keywords:
            current_scanners.append(
                BanSubstrings(substrings=custom_keywords, match_type="str")
            )
            ...
            for scanner_name, valid in results_valid.items():
                if not valid:
                    ...
                    if scanner_name == "Toxicity":
                        mapped_type = ViolationType.TOXICITY
                    elif scanner_name == "PromptInjection":
                        mapped_type = ViolationType.PROMPT_INJECTION
                    elif scanner_name == "Secrets":
                        mapped_type = ViolationType.SECRETS
                    elif scanner_name == "Code":
                        mapped_type = ViolationType.MALICIOUS_CODE
                    elif scanner_name == "BanSubstrings":
                        mapped_type = ViolationType.KEYWORD_FILTER
            ...
            for scanner_name, valid in results_valid.items():
                if not valid:
                    ...
                    if scanner_name == "Toxicity":
                        ...
                        mapped_type = ViolationType.PII  # Output PII
                    elif scanner_name == "Bias":
                        mapped_type = ViolationType.BIAS
                    elif scanner_name == "NoRefusal":
                        mapped_type = ViolationType.REFUSAL
                    elif scanner_name == "Relevance":
                        mapped_type = ViolationType.RELEVANCE
                    elif scanner_name == "BanSubstrings":
                        mapped_type = ViolationType.KEYWORD_FILTER
