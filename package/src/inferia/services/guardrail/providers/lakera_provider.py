...
class LakeraProvider(GuardrailProvider):
    ...
    def refresh_config(self):
        ...
        if not current_key:
            # Lakera doesn't have a global settings hydration like others yet
            pass
    ...
    async def scan_input(
        ...
        try:
                ...
                if res.get("flagged"):
                    ...
                    violations.append(
                        Violation(
                            ...
                            details=f"Lakera detected {category.replace('_', ' ')}"
                            if category
                            else "Lakera detected threat",
                        )
                    )
            ...
            if not results_list and result.get("flagged"):
                ...
                violations.append(
                    Violation(
                        ...
                        details=f"Lakera detected {category.replace('_', ' ')}"
                        if category
                        else "Lakera detected violation",
                    )
                )
            ...
            if allowed_scanners is not None:
                violations = [
                    v for v in violations if v.violation_type in allowed_scanners
                ]
    ...
    def _map_category(self, category: str) -> ViolationType:
        ...
        if "hate" in category:
            return ViolationType.HATE
        if "viol" in category:
            return ViolationType.VIOLENT_CRIMES
