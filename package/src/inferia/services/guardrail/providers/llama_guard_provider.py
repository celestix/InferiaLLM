...
class LlamaGuardProvider(GuardrailProvider):
    ...
    def initialize_client(self):
        ...
        if not current_key:
            if self.groq_client:
                logger.warning(
                    "[LlamaGuardProvider] Groq API key removed/missing. Client invalidated."
                )
                self.groq_client = None
                self._last_key = None
            return
        ...
        if Groq:
            try:
                ...
                logger.info(
                    "[LlamaGuardProvider] Groq client initialized/updated with key starting with "
                    + current_key[:6]
                )
            except Exception as e:
                logger.error(
                    f"[LlamaGuardProvider] Failed to initialize Groq client: {e}"
                )
        else:
            logger.error(
                "[LlamaGuardProvider] Groq library not installed. Cannot initialize client."
            )
    ...
    async def scan_input(
        ...
        if not self.groq_client:
            logger.error(
                "Llama Guard requested but Groq client is not initialized (missing API key)."
            )
        ...
        try:
            ...
            return self._parse_llama_guard_response(
                response, text, scan_time_ms, "input", config
            )
    ...
    async def scan_output(
        ...
        try:
            ...
            return self._parse_llama_guard_response(
                response, output, scan_time_ms, "output"
            )
    ...
    def _parse_llama_guard_response(
        ...
        if not response:
            ...
            return GuardrailResult(
                is_valid=True, sanitized_text=text, scan_time_ms=scan_time_ms
            )
